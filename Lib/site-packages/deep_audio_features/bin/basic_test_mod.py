    ...
    if verbose:
        if task == "classification":
            if layers_dropped == 0:
                ...
                # Print only the class name

                emotion_result = class_names[max_index]
                ...
                if status is not None:
                    ...
                    if (switch_led_status) == False:
                        #print("Turning the LED light to ON, device id: ", DEVICE_ID, ", model name: ", BULB_MODEL_NAME)
                        turn_on = {'commands': [{'code': 'switch_led', 'value': True}]}
                        openapi.post('/v1.0/iot-03/devices/{}/commands'.format(DEVICE_ID), turn_on)                                        

                #Get the emotion result
                emotion = class_names[max_index]

                if emotion == "happy":                    
                    color_data = { "h": 60, "s": 900, "v": 800 }  # for light yellow                    
                elif emotion == "anger":
                    color_data = { "h": 240, "s": 1000, "v": 1000 }  # for blue                    
                elif emotion == "sad":
                    color_data = { "h": 60, "s": 800, "v": 800 }  # for pale yellow     
                    #60°, 35%, 100%               
                else:
                    color_data = { "h": 35, "s": 700, "v": 800 }  # for beige white (adjust as needed)

                #Change the color command                                          
                command = {'commands': [{'code': 'colour_data_v2', 'value': color_data}]}
                ...
                message = {
                    ...
                    "start_time": True,
                    "complete": True  # Add the 'complete' property and set it to True to indicate that the script has finished
                }
...
if __name__ == '__main__':
    ...
    model = '/home/allanregistos/apps/emotiond/pkl/final1.pt'  # Replace with the actual model path
    ifile = '/home/allanregistos/apps/emotiond/test1.wav'                   
